...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    if (tag == 13) {
        if (section == SSID_SECTION) {
            ...
            if ([self.security isEqualToString:@"open"] || [self.security isEqualToString:@"none"]) {
                return 1;
            }
            else {
                return 3;
...
- (void)changeSecurityType
{
    ...
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
        step07ViewController = [[Step_07_ViewController alloc] initWithNibName:@"Step_07_ViewController_ipad" bundle:nil];
    }
    else {
        step07ViewController = [[Step_07_ViewController alloc] initWithNibName:@"Step_07_ViewController" bundle:nil];
    }
...
-(void) prepareWifiInfo
{
    ...
    NSString *stringUDID = [[HttpCom instance].comWithDevice sendCommandAndBlock:GET_UDID
                                                           withTimeout:5.0];
...
-(void)sendWifiInfoToCamera
{
    ...
    NSString * setup_cmd = [NSString stringWithFormat:@"%@%@", SETUP_HTTP_CMD, device_configuration];
...
- (void)showDialogPasswordWrong
{
    ...
    //ERROR condition
    UIAlertView *alertViewPassword = [[UIAlertView alloc] initWithTitle:@"Confirm Password Failed"
